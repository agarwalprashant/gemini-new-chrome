...
function createSidePanelButton() {
    ...
    if (document.getElementById('gemini-live-chat-toggle')) {
        return;
    }

    const button = document.createElement('button');
    button.id = 'gemini-live-chat-toggle';
    button.innerHTML = `
    ...
    button.title = 'Open Gemini Live Chat';
    button.setAttribute('aria-label', 'Toggle Gemini Live Chat Assistant');
    ...
    function handleActivation(e) {
        ...
        try {
            ...
            console.error("Extension context invalidated, please refresh the page:", e.message);
            button.style.pointerEvents = 'auto';
        }
    }
    ...
    if (!document.getElementById('gemini-live-chat-styles')) {
        const style = document.createElement('style');
        style.id = 'gemini-live-chat-styles';
        style.textContent = `
            @keyframes ripple {
                to {
                    transform: scale(4);
            ...
            #gemini-live-chat-toggle:active {
    ...
    console.log("Modern Gemini Live Chat toggle button injected into page.");
}

// Update toggle button visual state
function updateToggleButtonState(isOpen) {
    const button = document.getElementById('gemini-live-chat-toggle');
    if (!button) return;
    ...
    if (isOpen) {
        ...
        button.title = 'Close Gemini Live Chat';
    } else {
